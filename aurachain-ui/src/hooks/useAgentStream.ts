...
export const useAgentStream = (sessionId: string | null): UseAgentStreamReturn => {
  ...
  useEffect(() => {
    ...
    eventSource.onmessage = (event) => {
      try {
        ...
        switch (update.type) {
          ...
          case 'report_ready':
            if (update.data?.workflow_id) {
              handleReportReady(
                update.data.workflow_id,
                update.data.overall_confidence || 0
              );
            }
            break;
    ...
    return cleanup;
  }, [sessionId, updateAgentStatus, updateAgentProgress, updateAgentData, setRightPanelOpen, handleWorkflowStarted, handleWorkflowCompleted, handleReportReady]);
