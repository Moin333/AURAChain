...
% ============================================================
\begin{abstract}
Micro, Small, and Medium Enterprises (MSMEs) contribute substantially to global GDP yet remain critically underserved by modern supply chain intelligence tools. Existing solutions are either prohibitively expensive, require deep domain expertise, or lack the adaptability needed for volatile MSME operating environments. This paper presents AURAChain, an AI-native multi-agent platform that democratizes advanced supply chain decision-making through a collaborative agent ecosystem. The system introduces several technical contributions: (i)~a DAG-based workflow orchestration engine that dynamically composes heterogeneous AI agents through semantic intent analysis, (ii)~a ReAct-augmented reasoning loop integrated within specialized agents for autonomous tool selection and self-evaluation, (iii)~a hybrid forecasting-optimization pipeline combining Facebook Prophet with Monte Carlo Tree Search (MCTS) enhanced by progressive widening and UCB1-variant exploration, and (iv)~a closed-loop decision intelligence layer with structured reasoning traces and outcome tracking. Experimental evaluation on retail supply chain datasets demonstrates that AURAChain achieves a Mean Absolute Percentage Error (MAPE) of 10.1\% in demand forecasting, reduces inventory holding costs by 18.4\% compared to (s, S) policy baselines, and attenuates the Bullwhip Effect coefficient from 2.18 to 1.31. These results indicate that LLM-orchestrated multi-agent architectures can deliver enterprise-grade supply chain intelligence at a fraction of traditional deployment costs.
\end{abstract}
...
Despite the proliferation of AI-driven supply chain solutions, existing systems largely fail to address the specific challenges faced by MSMEs. Traditional enterprise AI platforms require massive volumes of structured historical data, specialized cloud infrastructure, and dedicated data science teams for deployment and continuous maintenance. Furthermore, these monolithic systems often operate as "black boxes," generating rigid forecasts without providing actionable context or explainability. For resource-constrained MSMEs dealing with sparse transaction histories, noisy data, and highly dynamic local market conditions, such heavyweight solutions are not only cost-prohibitive but fundamentally misaligned with their operational realities.
...
Notifier & Single-Shot & Discord webhook alert dispatch \\
\bottomrule
\end{tabular}
\end{table*}
...
\begin{enumerate}
    \item \textbf{Diwali Sales Dataset}~\cite{diwali_sales}: A publicly available Kaggle dataset (\texttt{Diwali Sales DataSet.csv}) containing sales information during the Diwali festival period in India, utilized to evaluate the system's ability to extract market trends, customer demographics, and seasonal spending peaks.
    
    \item \textbf{SuperMarket Analysis Dataset}~\cite{supermarket_analysis}: A Kaggle dataset (\texttt{SuperMarket Analysis.csv}) containing comprehensive supermarket transaction records, utilized to test the system's capability in sales trend analysis, customer segmentation, and market basket analysis across varied product lines.
    
    \item \textbf{Indian MSME Retail Dataset}~\cite{msme_synthetic}: A custom-designed synthetic dataset (\texttt{indian\_msmE\_retail\_dataset\_50k.csv}) containing 50,000 records tailored specifically for the Indian Micro, Small, and Medium Enterprise (MSME) context, created to stress-test the entire multi-agent system at its full operational potential.
\end{enumerate}
...
This section presents the experimental results obtained by evaluating AURAChain on the three datasets described in Section~IV. All forecasting metrics are computed using walk-forward temporal validation, and inventory optimization results are averaged over 50 independent simulation runs to ensure statistical stability.
...
\midrule
Naive Persistence & 26.1 & 51.7 & --- \\
ARIMA(1,1,1) & 16.8 & 35.2 & 88.4 \\
Single-Agent LLM & 19.4 & 40.1 & 81.7 \\
Prophet (default) & 12.3 & 26.4 & 92.8 \\
\textbf{AURAChain (Prophet+)} & \textbf{10.1} & \textbf{22.3} & \textbf{94.6} \\
\bottomrule
\end{tabular}
\end{table}

AURAChain's enhanced Prophet implementation achieves a MAPE of 10.1\%, outperforming default Prophet (12.3\%) through autonomous FFT-based seasonality detection and locale-aware holiday modeling. The improvement of 2.2 percentage points is consistent with findings in the Prophet tuning literature~\cite{taylor2018prophet}. The 95\% prediction interval coverage of 94.6\% indicates well-calibrated uncertainty estimates.
...
\begin{tabular}{@{}lrrrr@{}}
\toprule
\textbf{Policy} & \textbf{Cost (INR)} & \textbf{Red. (\%)} & \textbf{Svc. (\%)} & \textbf{S/O (\%)} \\
\midrule
No Reorder & 12,840 & --- & 41.3 & 58.7 \\
ROP (fixed) & 7,610 & 40.7 & 86.1 & 13.9 \\
(s, S) Policy & 6,920 & 46.1 & 88.7 & 11.3 \\
\textbf{AURAChain MCTS} & \textbf{5,650} & \textbf{56.0} & \textbf{91.4} & \textbf{8.6} \\
\bottomrule
\end{tabular}
\end{table}

The MCTS optimizer achieves an 18.4\% cost reduction over the (s, S) policy baseline and a 25.7\% reduction over fixed ROP, with a service level of 91.4\%. The progressive widening mechanism enables exploration of the continuous order quantity space, finding solutions that fixed threshold policies cannot reach.
...
\midrule
No Policy (baseline) & 2.18 & --- \\
ROP (fixed) & 1.79 & 17.9 \\
(s, S) Policy & 1.58 & 27.5 \\
\textbf{AURAChain MCTS} & \textbf{1.31} & \textbf{39.9} \\
\bottomrule
\end{tabular}
\end{table}

AURAChain reduces the Bullwhip coefficient from 2.18 to 1.31, a 39.9\% improvement over the unmanaged baseline. While this does not approach the theoretical minimum of 1.0, the reduction is practically significant and consistent with the smoother order patterns produced by MCTS-guided decisions compared to threshold-based policies.
...
\midrule
Full AURAChain & 10.1 & 56.0 \\
$-$ FFT Seasonality Detection & 11.8 & 54.7 \\
$-$ Holiday Modeling & 11.2 & 55.3 \\
$-$ Progressive Widening & 10.3 & 50.1 \\
$-$ Forecast-Scaled Demand & 10.2 & 52.8 \\
$-$ ReAct EDA Pre-processing & 10.9 & 55.6 \\
$-$ Cross-Agent Enrichment & 10.7 & 52.3 \\
\bottomrule
\end{tabular}
\end{table}

The ablation study reveals that component contributions are uneven, as expected. FFT-based seasonality detection provides the largest forecasting improvement (1.7 percentage points of MAPE), consistent with the strongly seasonal Diwali dataset. Progressive widening delivers the highest optimization gain (5.9 percentage points of cost reduction), confirming the value of continuous action exploration over fixed discretization. Notably, holiday modeling and ReAct EDA pre-processing show modest individual contributions, suggesting their value is partially captured by other components. Cross-agent enrichment --- where the Forecaster consumes Trend Analyst findings and the MCTS Optimizer scales demand by forecast ratios --- contributes 3.7 percentage points to cost reduction.
...
\begin{enumerate}
    ...
    \item \textbf{Dataset Diversity}: The current evaluation is limited to three datasets. Rigorous evaluation on a broader set of real-world MSME datasets with verified ground-truth outcomes is needed to validate production readiness.
...
The experimental evaluation demonstrates competitive forecasting accuracy (10.1\% MAPE), meaningful inventory cost reduction (18.4\% over (s, S) policy baselines), and significant Bullwhip Effect attenuation (39.9\% improvement). The ReAct-augmented reasoning loop enables agents to autonomously select and invoke analytical tools, while the decision intelligence layer creates an auditable feedback loop for continuous improvement.
...
\bibitem{diwali_sales}
Kaggle, ``Diwali Sales Data,'' 2023. [Online]. Available: \url{https://www.kaggle.com/}

\bibitem{supermarket_analysis}
Kaggle, ``Supermarket Sales Data,'' 2019. [Online]. Available: \url{https://www.kaggle.com/}

\bibitem{msme_synthetic}
AURAChain Research, ``Indian MSME Retail Synthetic Dataset (50k),'' 2024.
